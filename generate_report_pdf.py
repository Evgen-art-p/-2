...
def _register_fonts():
    ...
    # Возможные расположения DejaVu
    candidates = [
        _P("/usr/share/fonts/truetype/dejavu"),           # Linux
        _P("C:/Windows/Fonts"),                            # Windows (DejaVu если установлен)
        _P("/usr/share/fonts/TTF"),                        # Arch
        _P("/Library/Fonts"),                              # macOS
    ]
    ...
    if dejavu_dir:
        pdfmetrics.registerFont(TTFont("DejaVu", str(dejavu_dir / "DejaVuSans.ttf")))
        pdfmetrics.registerFont(TTFont("DejaVuBd", str(dejavu_dir / "DejaVuSans-Bold.ttf")))
        pdfmetrics.registerFont(TTFont("DejaVuIt", str(dejavu_dir / "DejaVuSans-Oblique.ttf")))
        pdfmetrics.registerFont(TTFont("DejaVuBI", str(dejavu_dir / "DejaVuSans-BoldOblique.ttf")))
        from reportlab.lib.fonts import addMapping
        addMapping("DejaVu", 0, 0, "DejaVu")
        addMapping("DejaVu", 1, 0, "DejaVuBd")
        addMapping("DejaVu", 0, 1, "DejaVuIt")
        addMapping("DejaVu", 1, 1, "DejaVuBI")
        return "DejaVu"
    else:
        # Фоллбэк: пробуем установить из pip
        try:
            import subprocess, sys
            subprocess.check_call([sys.executable, "-m", "pip", "install", "dejavu-fonts-ttf",
                                   "--break-system-packages", "-q"],
                                  stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except Exception:
            pass
        print("  ⚠️ DejaVu не найден — кириллица может отображаться квадратиками")
        return "Helvetica"
...
def _build_styles():
    ...
    styles.add(ParagraphStyle(
        ...
        fontName=f"{font}Bd" if font == "DejaVu" else "Helvetica-Bold",
        fontSize=14,
        textColor=C_ACCENT,
        spaceBefore=20,
        spaceAfter=8,
    ...
    styles.add(ParagraphStyle(
        "Dialogue",
        parent=styles["Normal"],
        fontName=f"{font}It" if font == "DejaVu" else "Helvetica-Oblique",
        fontSize=9,
        leading=13,
        textColor=C_TEXT,
        leftIndent=12,
