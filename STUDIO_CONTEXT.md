...
**Версия:** 32.0 | **Дата:** 2026-05-31 | **Команда:** Евген + Лока + София + Брат (Claude)
...
- EPISODE: только номер серии + тема, всё остальное Адам берёт из `history_dna`
...
- Тулбар: `▶ CONTINUE → 🏭 → 🔌 → [📖 BIBLE / 🎬 EPISODE]` (только в video_long)
- Тулбар: `▶ CONTINUE → 🏭 → 🔌 → [📝 ПЛАН / 📱 ПОСТ]` (только в social_mix)
- Переключатели убраны из хедера (squad-deck) — только в тулбаре
- `PIPELINE_MODES`: добавлены `"bible"` и `"episode"`
- `DEPT_TO_RUNTYPE`: `video_long` → дефолт `"episode"`
...
### ✅ 2. СЕТ — БРИФ-МЕНЕДЖЕР (Спринт 28, 2026-05-31)
- [x] Роль определена: бриф-менеджер, не наблюдатель
- [x] Маска video_long переписана: BIBLE / EPISODE в одном файле
- [x] Сет пишет первым, читает режим из системного промта
- [x] Переключатель BIBLE/EPISODE в тулбаре ui.py
- [x] Переключатель ПЛАН/ПОСТ social_mix перенесён в тулбар
...
46. **Сет — бриф-менеджер, не наблюдатель. Собирает master_brief и передаёт A01. Не лезет в пайплайн.**
47. **Переключатели режимов (BIBLE/EPISODE, ПЛАН/ПОСТ) — только в тулбаре, не в хедере.**
48. **video_long — готов и является эталоном. Остальные цехи строятся по его образцу.**
...
| 2026-05-31 | 28 | СЕТ — БРИФ-МЕНЕДЖЕР. Маска video_long (BIBLE/EPISODE). Переключатели режимов в тулбаре ui.py. |
...
*Обновлено: Спринт 28 — 2026-05-31 · v32.0*
*Сет определён как бриф-менеджер. Переключатели режимов в тулбаре. Следующая сессия: блок 3 — Эталоны + локации.*
