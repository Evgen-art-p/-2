...
**Версия:** 31.0 | **Дата:** 2026-05-30 | **Команда:** Евген + Лока + София + Брат (Claude)
...
### 🔴 1. МОНТАЖЁР — ЗАВЕРШИТЬ ПОЛНОСТЬЮ (Спринт 28)
- [ ] **Первый реальный ран** — video_long, всё готово!
- [ ] **Монтажёр через Страницу Жизни** — родить резидента, аватар, промт, DNA
...
### 🟡 2. СЕТ — СМОТРЯЩИЙ НА VIDEO_LONG
- [ ] Определить полномочия Сета (сейчас ⏳ не определены)
- [ ] Роль: внешний наблюдатель за цепочкой video_long?
- [ ] DNA, промт, Страница Жизни
- [ ] Связь с hooks.py video_long

---

### 🟡 3. ЭТАЛОНЫ + ОСТАТОК ЛОКАЦИЙ
- [ ] **Эталоны (Self-QA):** `library/visual_benchmarks/` + `sound_benchmarks/`
- [ ] Self-QA в манифестах цехов: `self_qa.enabled`, `benchmark_slot`, `retry_on: далеко`
- [ ] Кабинет: самооценка агента + кнопка ✅/❌ → `global_feedback.json`
- [ ] Turbo/social_mix — полный автомат, без эталонов (Министерство отбирает)
- [ ] **Локации:** добавить оставшиеся до полного каталога (сейчас 13 из планируемых)

---

### 🟡 4. ЖИВОЙ ГОРОД — ВСЕ ОСТАТКИ
- [ ] **GENERATE_INTENTS = True** — включить после первого рана
- [ ] Инерция привычки: `location_affinity` накапливается в DNA агента
- [ ] Личные ритуалы по DNA (Джем → Таверна, А05 → тихие локации)
- [ ] Живые события: локальные слухи + глобальные от Садовника
- [ ] Погода из стресса (убрать рандом из `city_walker.py`)
- [ ] Эмоции влияют на пайплайн: `trust` и `resentment` → `pipeline.py`
- [ ] Диалоги в UI Кабинета (топ 1–3 встречи за день → Gemini Flash → чат)
- [ ] Джем — определить полномочия

---

### 🟡 5. ПРОМТЫ БАЗОВЫХ ЦЕХОВ
- [ ] **turbo** — 5 агентов (манифест ✅, hooks ✅, промты ⏳)
- [ ] **social_mix** — 12 агентов (манифест ✅, hooks ✅, промты ⏳)
- [ ] **video_shorts** — проверить промты по стандарту Спринта 26
- [ ] По стандарту: IDENTITY / INPUT / KNOWLEDGE BASE / TASK / OUTPUT / RULES

---

### 🟠 6. СММ АДМИНИСТРАТОР (smm_admin)
- [ ] Черновик правил готов: `SMM_ADMIN_RULES.md` v0.1 (2026-05-30)
- [ ] Ответить на открытые вопросы (см. SMM_ADMIN_RULES §9)
- [ ] Приоритет платформ для 007_ADMAN (реклама API)
- [ ] manifest.json, CHAIN_CONTRACT, hooks.py
- [ ] Промты A01–A07 по стандарту Спринта 26
- [ ] KB для Юриста (legal_RF, legal_EU, platform_meta и др.)
- [ ] 007_ADMAN — резидент, аналог 006_MONTEUR

---

### 🟢 7. ВСЁ ОСТАЛЬНОЕ
- [ ] Манифесты 7 оставшихся цехов до v2.0 (web_story, clipmakers, advertising, market_hit, logo_design, emo_card, living_book)
- [ ] Agent Factory
- [ ] Аудиофайлы Foley
- [ ] Деплой Hetzner
...
| 2026-05-30 | — | ПЛАНИРОВАНИЕ. Беклог переупорядочен по приоритетам. SMM_ADMIN_RULES v0.1 — черновик нового цеха (7 агентов + резидент 007_ADMAN). |
...
*Обновлено: Планирование — 2026-05-30 · v31.0*
*Беклог переупорядочен. SMM_ADMIN_RULES v0.1 зафиксирован. Следующая сессия: Спринт 28 — Монтажёр полностью.*
