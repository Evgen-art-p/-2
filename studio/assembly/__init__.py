...
def _find_projects() -> list[dict]:
    ...
    for run_dir in sorted(RUNS_DIR.iterdir(), reverse=True):
        ...
        slot_id = deliverables.get("slot_id", "")
...
def _render_shorts_workbench(el, d: dict, pid: str, assembly: dict):
    """
    ...
    def _refresh_queue():
        ...
        with el:
            if not state["projects"]:
                ui.html('<div class="mt-empty">Нет проектов<br>Запусти пайплайн — появятся здесь</div>')
                return
