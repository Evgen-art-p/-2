# studio/ui_reception.py
from nicegui import ui
from studio.modules_registry import load_depts
import json
...
def page_reception() -> None:
    ...
    depts_js = json.dumps([{
        ...
        "keywords": d.keywords,
        "priority": d.priority,
    } for d in depts], ensure_ascii=False)
    ...
    <script>
      ...
      window.currentDept = (DEPTS[0]?.id || 'social_mix');
      ...
      function smartPick(text){
        ...
        if (!t) return (DEPTS[0]?.id || 'social_mix');
        let best = null;
        ...
        if (!best || best.score === 0) return (DEPTS[0]?.id) || 'social_mix';
        return best.id;
      }
      ...
      function goWorkspace(deptId){
        ...
        setTimeout(() => {
          const url = '/workshop?dept=' + encodeURIComponent(deptId) + '&prompt=' + encodeURIComponent(prompt);
    ...
    def hud():
        ...
        with ui.element('div').classes('hud' + (' is-busy' if busy else '')):
            with ui.element('div').classes('hud_capsule'):
                  ...
                  <div class="suggest_wrap" id="dept_suggest_wrap">
                    <div class="suggest_card" id="dept_suggest" onclick="acceptSuggest()">
                      <div class="suggest_left">
                        ...
                        <div>
                          <div class="suggest_title" id="suggest_title">Hype Factory</div>
                          <div class="suggest_sub" id="suggest_sub">Похоже на соцсети. Открываю Хайп-Цех?</div>
                        </div>
                      </div>
                ...
                with cabinet_btn:
                    ui.icon('smart_toy')
                cabinet_btn.on('click', lambda: ui.navigate.to('/cabinet'))

                inp = ui.input(placeholder='Чего желаете, Шеф?').props('borderless dense').classes('w-full')
                inp.on('input', lambda e: ui.run_javascript(f"onPromptInput({(e.value or '')!r});"))

                folder = ui.element('div').classes('hud_folder')
                with folder:
                    ui.icon('folder_open')
                folder.on('click', lambda: ui.run_javascript('enableManualLock(window.currentDept); toggleDeck(true);'))
