...
FINCH_DIR   = RESIDENTS_DIR / "007_FINCH"
...
def _load_finch_prompt() -> str:
    """Загружает промпт Финча из forge/prompt.md."""
    cache_key = "finch_prompt"
    if cache_key in _prompt_cache:
        return _prompt_cache[cache_key]
    path = FINCH_DIR / "forge" / "prompt.md"
    result = path.read_text(encoding="utf-8") if path.exists() else ""
    _prompt_cache[cache_key] = result
    return result
...
def get_finch_system_prompt() -> str:
    """Системный промпт Финча для LLM-вызовов."""
    return _load_finch_prompt()


def invalidate_finch_cache():
    """Сбросить кеш промпта Финча."""
    global _prompt_cache
    _prompt_cache.pop("finch_prompt", None)


def run_finch_morning(on_progress=None) -> dict:
...
def get_finch_garden_state() -> dict:
    """
    Возвращает текущее состояние сада для Кабинета.
    seeds / growing / ole / compost / total
    """
    try:
        from studio.garden_tools import get_garden_state
        return get_garden_state()
    except Exception as e:
        print(f"[ФИНЧ] ❌ get_finch_garden_state: {e}")
        return {"seeds": [], "growing": [], "ole": [], "compost": [], "total": 0}
...
def finch_plant(
    ...
    """
    Посадить семя в сад Финча.
    Может вызвать любой субъект города.
    """
    try:
        from studio.garden_tools import plant
        return plant(
...
def finch_return_to(
    ...
    """
    Зафиксировать возвращение к артефакту.
    Возвращение ≠ просмотр. Только осознанное повторное обращение.
    """
    try:
        from studio.garden_tools import return_to
        return return_to(
            artifact_id=artifact_id,
        ...
        print(f"[ФИНЧ] ❌ finch_return_to: {e}")
        return False
