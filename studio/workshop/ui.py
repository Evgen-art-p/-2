...
from studio.modules_registry import CURRENT_DEPT, get_worker_prompt, get_worker_info, get_worker_knowledge, get_worker_home, format_worker_state
...
def page_workshop(dept: str = 'video_long', prompt: str = '') -> None:
    ...
    async def continue_pipeline():
        """Продолжить пайплайн после checkpoint"""
        if not state["paused_at"]:
            ui.notify("Нет паузы — нечего продолжать!", type="warning")
            return
        
        resume_from = state["paused_at"]
        ctx = state["paused_context"]
        
        # Очищаем checkpoint
        state["paused_at"] = None

        # ── ФИКС: пересобираем previous_output из актуальных results ──
        # (старый paused_output содержит сценарий до правки — выбрасываем)
        all_agents_flat = [w for workers in WORKERS.values() for w in workers]
        rebuilt_output = ""

        for wid in all_agents_flat:
            if wid == resume_from:
                break  # дошли до точки паузы — стоп
        ...
        print(f"[CONTINUE] Возобновление с {resume_from}")
        
        # Запускаем pipeline с места остановки
        await run_pipeline(
            from_worker=resume_from,
            with_chat_context=ctx.get("with_chat_context", False)
        )


    # ─── TURBO PIPELINE (A02∥A03 параллельно) ────────────
    async def turbo_pipeline():
        ...
        update_status()
        ui.notify("⚡ TURBO запущен!", type='info')
        ...
        # Захватываем клиентский контекст ДО параллельных тасков
        _client = ui.context.client

        async def _call_one(worker_id, previous_output, tag=""):
            """Вызов одного TURBO-агента"""
            # Оборачиваем ВСЕ UI-вызовы в клиентский контекст
            with _client:
            ...
            # Каталог ассетов
            _catalog_block = _load_asset_catalog()
            if _catalog_block:
                context += f"\n\n{_catalog_block}\n\n"
            ...
            if _GRONDHEIM_ENABLED:
                ...
                if _soul:
                    context += _soul + "\n\n"
                    _soul_preview = _soul.replace('\n', ' ')[:120]
                    print(f"🌀 {worker_id} [{_dept}] просыпается: {_soul_preview}")

            # Грондхейм: петля обратной связи (streak/stress → промпт)
            _boost = _grondheim_context_boost(worker_id, state.get("active_dept", ""))
            if _boost:
                context += _boost + "\n\n"
            ...
            if client_slug != "_sandbox":
                context += "\n=== ИНСТРУКЦИЯ ===\nВ конце ответа: INSIGHT: <вывод о клиенте>\n"

            vision_images = _collect_images_for_vision(state)
            _temp = _grondheim_temperature(worker_id, state.get("active_dept", ""))
            if vision_images:
                raw_result = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda sp=system_prompt, ctx=context, wk=worker_knowledge, vi=vision_images, t=_temp:
                        chat_with_images(sp, ctx, images=vi, knowledge=wk, temperature=t)
                )
            else:
                raw_result = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda sp=system_prompt, ctx=context, wk=worker_knowledge, t=_temp:
            ...
            _ghost_ids = _validate_asset_ids(meta, worker_id)
            if _ghost_ids:
                _warn = f"⚠️ {worker_id}: галлюцинации asset_id ({len(_ghost_ids)}): " + ", ".join(_ghost_ids[:5])
                print(f"[VALIDATION] {_warn}")
                with _client:
                    ui.notify(_warn, type='warning', timeout=8000)

            # ── Auto-retry: галлюцинации + плохая серия → перезапуск ──
            if _ghost_ids and _GRONDHEIM_ENABLED and not state.get(f"_retried_{worker_id}"):
                try:
                    from studio.grondheim_memory import _find_agent_dir, _load_json
                    _ad = _find_agent_dir(worker_id, state.get("active_dept", ""))
                    _dna = _load_json(_ad / "dna.json") if _ad else {}
                    _streak = int(_dna.get("dynamic", {}).get("streak", 0))
                    if _streak <= -1 or len(_ghost_ids) >= 3:
                        state[f"_retried_{worker_id}"] = True
                        _retry_ctx = context + (
                            "\n\n⚠️ ПОВТОРНЫЙ ЗАПУСК: предыдущий результат содержал "
                            f"выдуманные asset_id: {', '.join(_ghost_ids[:5])}. "
                            "ИСПОЛЬЗУЙ ТОЛЬКО ID ИЗ КАТАЛОГА ВЫШЕ. "
                            "Если нужного персонажа нет в каталоге — опиши его словами, НЕ ВЫДУМЫВАЙ ID."
                        )
                        print(f"🔁 {worker_id} RETRY: {len(_ghost_ids)} галлюцинаций, streak={_streak}")
                        if vision_images:
                            raw_result = await asyncio.get_event_loop().run_in_executor(
                                None,
                                lambda sp=system_prompt, ctx=_retry_ctx, wk=worker_knowledge, vi=vision_images:
                                    chat_with_images(sp, ctx, images=vi, knowledge=wk)
                            )
                        else:
                            raw_result = await asyncio.get_event_loop().run_in_executor(
                                None,
                                lambda sp=system_prompt, ctx=_retry_ctx, wk=worker_knowledge:
                                    chat(sp, ctx, wk)
                            )
                        human_text, meta = parse_agent_response(raw_result)
                        human_text = _clean_response(human_text)
                        _ghost_ids_2 = _validate_asset_ids(meta, worker_id)
                        if _ghost_ids_2:
                            print(f"🔁 {worker_id} RETRY: ещё {len(_ghost_ids_2)} галлюцинаций (не помогло)")
                        else:
                            _ghost_ids = []
                            print(f"🔁 {worker_id} RETRY: ✅ чисто!")
                except Exception as _retry_err:
                    print(f"[RETRY] {worker_id}: ошибка — {_retry_err}")
            ...
            state["results"][worker_id] = {"text": human_text, "meta": meta, "raw": raw_result}

            # Грондхейм: запись результата
            if _GRONDHEIM_ENABLED:
                _dept = state.get("active_dept", "")
                # Умный quality: deliverables есть и чистые → 0.8, галлюцинации → 0.2, остальное → 0.5
                if _ghost_ids:
                    _q = 0.2
                elif meta.get("deliverables") or meta.get("my_output"):
                    _q = 0.8
                else:
                    _q = 0.5
                on_agent_done(worker_id, human_text[:200], _q, _dept)
                print(f"✅ {worker_id} [{_dept}] завершил: q={_q:.1f} | {human_text[:80]}")
            ...
            if my_output:
                try:
                    ...
                    pass
            chunk = meta.get("next_input") or (human_text[:800] + chain_json)
            new_prev = previous_output + f"\n\n--- {label} ({worker_id}) ---\n{chunk}"

            # UI-обновления снова через клиентский контекст
            with _client:
                _viewer_header = ""
                if _GRONDHEIM_ENABLED:
                    _parts = []
                    if _soul:
                        _first_line = _soul.split('\n')[0].replace('=== ⚓ ЯКОРЯ ИДЕНТИЧНОСТИ (неизменяемые) ===', '').strip()
                        if _first_line:
                            _parts.append(f"💭 *{_first_line[:80]}*")
                    if _temp is not None and _temp != 0.7:
                        _parts.append(f"🌡️ {_temp}")
                    if _boost:
                        _boost_short = _boost.split('\n')[1][:60] if '\n' in _boost else _boost[:60]
                        _parts.append(f"⚡ {_boost_short}")
                    if _parts:
                        _viewer_header = "> " + " · ".join(_parts) + "\n\n"
                update_viewer(f"# {label} ({worker_id})\n\n{_viewer_header}{human_text}")
                if worker_id in avatars_ref['elements']:
            ...
            return human_text, new_prev

        # ═══ EXECUTION ═══
        previous_output = ""
        try:
            # STEP 1: A01
            _, previous_output = await _call_one("A01", previous_output)
            await asyncio.sleep(0.3)

            # STEP 2+3: A02 ∥ A03 ПАРАЛЛЕЛЬНО
            ui.notify("⚡ A02 + A03 — ПАРАЛЛЕЛЬНО!", type='info')
            prev_snap = previous_output

            results_pair = await asyncio.gather(
                _call_one("A02", prev_snap, " ⇄"),
                _call_one("A03", prev_snap, " ⇄"),
                return_exceptions=True
            )

            for i, aid in enumerate(["A02", "A03"]):
                if isinstance(results_pair[i], Exception):
                    ui.notify(f"❌ {aid}: {results_pair[i]}", type='negative')
                    if aid in avatars_ref['elements']:
                        avatars_ref['elements'][aid].classes(remove='working')
                else:
                    _, part = results_pair[i]
                    previous_output += part[len(prev_snap):]
            await asyncio.sleep(0.3)

            # STEP 4: A04
            _, previous_output = await _call_one("A04", previous_output)
            await asyncio.sleep(0.3)

            # STEP 5: A05
            _, previous_output = await _call_one("A05", previous_output)

        except Exception as e:
            ui.notify(f"❌ TURBO: {e}", type='negative')
            import traceback
            traceback.print_exc()
        ...
        update_status()
        update_runs_display()
        ui.notify("🎉 TURBO завершён!", type='positive')
        ...
        if client_slug != "_sandbox":
            try:
                ...
                results_text = "".join([
                    f"\n{wid}: {(r.get('text','') if isinstance(r,dict) else str(r))[:300]}"
                    for wid, r in state["results"].items()
                ])
                summary = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda: chat(
                        "Ты архивариус студии. Сожми сессию в конспект.",
                ...
                if state["project_dir"]:
                    (state["project_dir"] / "session_summary.md").write_text(
    ...
    async def run_pipeline(from_worker=None, with_chat_context=False):
        ...
        # Используем существующий project_dir если файлы уже загружены
        if not state["project_dir"] or not state["project_dir"].exists():
            _update_project_dir(create=True)
        else:
            # project_dir уже есть (файлы загружены) — убедимся что file_processor на месте
        ...
        start_index = 0
        if from_worker and from_worker != "SET":
            for i, (_, w) in enumerate(all_agents):
                if w == from_worker:
                    start_index = i
                    break
        ...
        if state["uploaded_files"] and state["file_processor"]:
            try:
                files_ctx = state["file_processor"].format_for_agent()
                print(f"[FILES] Контекст файлов: {len(files_ctx)} символов, файлов: {len(state['uploaded_files'])}")
                if not files_ctx.strip():
                    # Пробуем пересканировать assets
                    assets_dir = state["project_dir"] / "assets" if state["project_dir"] else None
                    if assets_dir and assets_dir.exists():
                        actual_files = list(assets_dir.iterdir())
                        print(f"[FILES] В папке assets: {[f.name for f in actual_files]}")
            except Exception as ex:
...
Duration: {state['settings']['duration']} sec
...
"""
        ...
        # Подхватываем previous_output если это continue после checkpoint
        if state["paused_output"] and from_worker:
            previous_output = state["paused_output"]
            state["paused_output"] = ""
            print(f"[CONTINUE] Подхватил previous_output: {len(previous_output)} символов")
        else:
            previous_output = ""
        
        for i, (shop_name, worker_id) in enumerate(all_agents):
            if i < start_index:
                continue
            ...
            try:
                ...
                if with_chat_context and state["chat_history"]:
                    last_msgs = state["chat_history"][-5:]
                    chat_context = "\n".join([
                        f"{m.get('role','user').upper()}: {m.get('content','')}"
                        for m in last_msgs
                    ])
                    anchor_ctx = (
                        f"=== КОНТЕКСТ ОБСУЖДЕНИЯ (последние {len(last_msgs)} сообщений) ===\n"
                        f"{chat_context}\n"
                        f"=== КОНЕЦ КОНТЕКСТА ОБСУЖДЕНИЯ ===\n\n"
                        f"ВАЖНО: Учитывай контекст обсуждения выше при выполнении задачи. "
                        f"Пользователь уже обсуждал детали — не игнорируй их.\n\n"
                    )
                else:
                    anchor_ctx = ""
                # ════════════════════════════════════
                ...
                # Контекст клиента для этого агента
                client_ctx = format_memory_for_agent(client_slug, worker_id)
                if client_ctx:
                    context += client_ctx + "\n\n"
                
                # Конспекты прошлых сессий
                session_ctx = format_session_context(client_slug)
                if session_ctx:
                    context += session_ctx + "\n\n"
                ...
                # Инструкция для записи в memory
                if client_slug != "_sandbox":
                    context += f"""
=== ИНСТРУКЦИЯ ===
В конце своего ответа добавь блок INSIGHT — одно предложение, ключевой вывод о клиенте,
который будет полезен тебе в будущих проектах с этим клиентом.
Формат: INSIGHT: <твой вывод>
"""
                
                # Vision: передаём изображения если есть
                vision_images = _collect_images_for_vision(state)
                _temp = _grondheim_temperature(worker_id, state.get("active_dept", ""))
                ...
                if vision_images:
                    ...
                    raw_result = await asyncio.get_event_loop().run_in_executor(
                        None,
                        lambda sp=system_prompt, ctx=context, wk=worker_knowledge, vi=vision_images, t=_temp: chat_with_images(sp, ctx, images=vi, knowledge=wk, temperature=t)
                    )
                else:
                    raw_result = await asyncio.get_event_loop().run_in_executor(
                        None,
                        lambda sp=system_prompt, ctx=context, wk=worker_knowledge, t=_temp: chat(sp, ctx, wk, temperature=t)
                    )
                ...
                # Извлекаем INSIGHT и записываем в memory
                insight_match = re.search(r'INSIGHT:\s*(.+)', raw_result)
                if insight_match and client_slug != "_sandbox":
                    insight_text = insight_match.group(1).strip()
                    append_to_memory(client_slug, run_date, run_type, worker_id, insight_text)
                    # Убираем INSIGHT из отображаемого текста
                    human_text = re.sub(r'\n*INSIGHT:\s*.+', '', human_text).strip()
                ...
                if _GRONDHEIM_ENABLED:
                  ...
                  # Feedback loop: Артур сохраняет оценки для следующего рана
                ...
                # Для следующего агента — передаём my_output с ассетами
                my_output = meta.get("my_output", {})
                chain_json = ""
                if my_output:
                    try:
                ...
                ui.notify(f"✅ {label} готов!", type='positive')
                
                await asyncio.sleep(0.5)

                # ══════ CHECKPOINT (Шлюз) ══════
                mode_config = PIPELINE_MODES.get(state["run_type"], {})
                stop_after = mode_config.get("stop_after")
                checkpoint_at = mode_config.get("checkpoint_at", [])
                checkpoint_after = mode_config.get("checkpoint_after", [])  # хард-стоп по имени агента
                agent_number = i + 1
                should_stop = (
                    (stop_after and agent_number >= stop_after) or
                    (agent_number in checkpoint_at) or
                    (worker_id in checkpoint_after)
                )
                print(f"[CHECKPOINT CHECK] agent={worker_id} num={agent_number} run_type={state['run_type']} stop_after={stop_after} checkpoint_at={checkpoint_at} checkpoint_after={checkpoint_after} → stop={should_stop}")
                if should_stop:
                    # Определяем следующий агент
                    next_idx = i + 1
                    next_worker = all_agents[next_idx][1] if next_idx < len(all_agents) else None
                    
                    # Сохраняем состояние для продолжения
                    state["paused_at"] = next_worker
                    state["paused_output"] = previous_output
                    state["paused_context"] = {
                        "files_ctx": files_ctx,
                        "settings_ctx": settings_ctx,
                        "client_slug": client_slug,
                        "run_date": run_date,
                        "run_type": run_type,
                        "with_chat_context": with_chat_context,
                    }
                    
                    state["pipeline_running"] = False
                    update_status()
                    
                    mode_label = mode_config.get("label", state["run_type"])
                    ui.notify(
                        f"⏸ CHECKPOINT после {worker_id}! Проверь результат, отредактируй если нужно. Жми ▶ CONTINUE для продолжения.",
                        type="warning",
                        timeout=0,
                        close_button="OK"
                    )
                    print(f"[CHECKPOINT] Пауза после {worker_id}. Continue начнёт с {next_worker}")
                    return  # Выходим из run_pipeline, но state сохранён
                # ════════════════════════════════════
                
            except Exception as e:
                ui.notify(f"❌ Ошибка {worker_id}: {e}", type='negative')
                if worker_id in avatars_ref['elements']:
                    avatars_ref['elements'][worker_id].classes(remove='working')
        ...
        if client_slug != "_sandbox":
            try:
                ui.notify("🧠 Сохраняю конспект сессии...", type='info')
                
                # Собираем всё что было в сессии
                chat_text = "\n".join([
                    f"{m.get('role','')}: {m.get('content','')[:200]}" 
                    for m in state["chat_history"][-20:]  # последние 20 сообщений
                ])
...
"""
                ...
                # Сохраняем конспект в файл run'а
                summary_file = state["project_dir"] / "session_summary.md"
                summary_file.write_text(f"# Конспект сессии\n\n{summary}", encoding='utf-8')
                
                ui.notify("🧠 Конспект сохранён в память!", type='positive')
                
            except Exception as e:
                ui.notify(f"⚠️ Суммаризация не удалась: {e}", type='warning')
    ...
    with ui.element('div').classes('app-container'):
        ...
        with ui.element('div').classes('area-header'):
            with ui.element('div').classes('glass squad-deck').style('display:flex; align-items:center; justify-content:center; width:100%; gap:8px;'):
                with ui.element('div').style('display:flex; align-items:center; gap:6px; flex-wrap:wrap; justify-content:center;'):
                ...
                # Тоггл режима — только для цеха соцсетей
                if dept == 'social_mix':
                    mode_refs = {}
                    with ui.element('div').style(
                    ...
                    with ui.element('div').classes('panel-body'):
                        ...
                        with ui.element('div').classes('setting-row'):
                            ...
                            ui.number(
                                ...
                                min=5,
                                max=300,
                                on_change=lambda e: state['settings'].update({'duration': int(e.value)})
                            ).style('width: 100%')
        ...
        with ui.element('div').classes('area-stage'):
            with ui.element('div').classes('glass stage-monitor').style('height:100%; overflow:hidden;'):
                ...
                with ui.element('div').classes('stage-toolbar').style('flex-shrink:0;'):
                    ...
                    # ── ЦЕНТРАЛЬНАЯ ГРУППА: CONTINUE | 🏭 ────────────────
                    with ui.element('div').style('display:flex; gap:6px; align-items:center; justify-content:center;'):
                        ui.button('▶ CONTINUE', on_click=lambda: continue_pipeline()).props('flat no-caps').style(
        ...
        with ui.element('div').classes('area-right'):
            with ui.element('div').classes('right-col'):
                ...
                ui.button(
                    ...
                    on_click=lambda: turbo_pipeline() if is_turbo else run_pipeline()
