# studio/modules/trading/brut_live.py
# ─────────────────────────────────────────────────────────────
# ЖИВОЙ ПРОГОН БРУТА (A06) — первый ТРЕЙДЕР Совета Биржи
# BRUT_ENGINE_V1 · Версия: 0.1 · 2026-06-18
#
# Форма — близнец сенсоров (morj_live / hans_live): живая модель +
# штатная память + душа города + двухслойный JSON + петля обучения.
#
...
#   · ТАБЛО  (trading_state["brut"]) — «сейчас», для Исполнителя 09.
#     перетирается каждый бар.
#   · ДНЕВНИК (state/diary_brut.jsonl) — событие во времени, КОПИТСЯ.
...
A06_DIR      = _HERE / "A06"
PROMPT_PATH  = A06_DIR / "forge" / "prompt.md"
KNOWLEDGE    = A06_DIR / "forge" / "knowledge" / "KOTIN_PHILOSOPHY.md"   # книга Котина (общая троим)
DNA_PATH     = A06_DIR / "dna.json"
...
def run_brut(symbol: str = "XAUUSD", timeframe: str = "H4",
    ...
    в табло (для 09) и открывает событие в дневнике.
    ...
    _save_verdict_to_table(signal)                       # ТАБЛО — для 09
