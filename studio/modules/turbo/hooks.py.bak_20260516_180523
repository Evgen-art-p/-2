...
# v3.1 — T3 генерит кадры, T5 генерит обложки.
...
def on_after_agent(state: dict, worker_id: str, human_text: str, meta: dict) -> dict:
    if worker_id == "T3":
        _t3_generate_keyframes(state, human_text)
    elif worker_id == "T5":
        _t5_generate_thumbnails_and_deliverables(state, human_text)
    return {}
...
def _t3_generate_keyframes(state: dict, human_text: str):
    """
    Парсит JSON из ответа T3.
    ...
    if not data:
        print("[T3] JSON не найден — пропускаю генерацию кадров")
        return
    ...
    if not frames:
        print("[T3] key_frames пуст — пропускаю генерацию")
        return
    ...
    for i, frame in enumerate(frames):
        ...
        if not prompt:
            print(f"[T3] Кадр {i+1}/{total}: нет banana_prompt — пропускаю")
            frame["path"] = None
            continue
        ...
        print(f"\n[T3] 🎬 Кадр {i+1}/{total}: {filename_base}")
        print(f"[T3]   Промпт: {prompt[:80]}...")
        if ref_ids:
            print(f"[T3]   Референсы: {ref_ids}")
        ...
        if path:
            generated += 1
            print(f"[T3] 💾 {dest} | score={score} | {quality}")
        else:
            print(f"[T3] ⚠️ Кадр {i+1} не сгенерирован после {MAX_ATTEMPTS} попыток")

    print(f"\n[T3] Итого: {generated}/{total} кадров сгенерировано")
...
def _t5_generate_thumbnails_and_deliverables(state: dict, human_text: str):
    """
    Парсит JSON из ответа T5.
    Генерирует variant_a и variant_b с проверкой качества и ретраями.
    Читает пути кадров из chain_data["vizor_visual"] (записанного T3).
    Собирает deliverables с готовыми путями.
    Пишет tasks[] для Assembly.
    """
    data = _parse_json(human_text)
    if not data:
        print("[T5] JSON не найден — пропускаю генерацию обложек")
        return
    ...
    if not thumb:
        print("[T5] thumbnail не найден — пропускаю генерацию обложек")
    ...
    for variant_name in ["variant_a", "variant_b"]:
        ...
        if not prompt:
            print(f"[T5] {variant_name}: нет banana_prompt — пропускаю")
            continue

        ref_ids = _norm_refs(variant.get("ref_ids", []))
        dest    = project_dir / f"thumb_{variant_name}.png"

        print(f"\n[T5] 🖼️ Обложка: {variant_name}")
        print(f"[T5]   Промпт: {prompt[:80]}...")
        if ref_ids:
            print(f"[T5]   Референсы: {ref_ids}")
        ...
        if path:
            generated += 1
            print(f"[T5] 💾 {dest} | score={score} | {quality}")
        else:
            print(f"[T5] ⚠️ {variant_name} не сгенерирован после {MAX_ATTEMPTS} попыток")

    print(f"\n[T5] Обложки: {generated}/2 сгенерировано")
    ...
    if not vizor_frames:
        print("[T5] ⚠️ vizor_visual.key_frames пуст — пути к кадрам отсутствуют")
    ...
    print(f"[T5] deliverables: {ready_frames}/{len(deliverables['key_frames'])} кадров, "
