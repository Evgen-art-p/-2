...
def dashboard_page():
    ...
    def render_council_detail():
        ...
        with el:
            if avatar:
                ui.html(
                    f'<div class="dashboard-detail-avatar" style="background-image:url(\'{ avatar }\')"></div>'
                )
            else:
                ui.html(
                    f'<div class="dashboard-detail-avatar" style="display:flex;align-items:center;justify-content:center;font-size:2rem;">{emoji}</div>'
                )
            ui.label(label).classes("dashboard-detail-name")
            ui.html(
                f'<div class="dashboard-detail-id" style="color:{color}">резидент совета</div>'
            )
            ...
            ui.html(
                f'<div style="font-family:JetBrains Mono;font-size:0.58rem;color:{status_color};margin-bottom:12px;">{status_text}</div>'
            )
            ...
            try:
                ...
                if memories:
                    ui.html(
                        f'<div style="font-family:JetBrains Mono;font-size:0.55rem;color:rgba(140,150,180,0.6);margin-bottom:8px;">🧠 помнит {len(memories)} разговоров с тобой</div>'
                    )
            except Exception:
                pass
            ...
            if state["council_chat"]:
                ...
                ui.html(
                    f'<div style="font-family:JetBrains Mono;font-size:0.56rem;color:rgba(140,150,180,0.5);">{len(state["council_chat"])} сообщений</div>'
                )
                ui.button(
                    "🗑 очистить",
    ...
    def render_council_chat():
        ...
        with el:
            if not state["council_chat"]:
                ui.html(
                    '<div style="text-align:center;padding:40px 20px;font-family:JetBrains Mono;font-size:0.6rem;color:rgba(140,150,180,0.3);">выбери резидента справа<br><span style="font-size:0.52rem;color:rgba(140,150,180,0.2);">он прочитает свои следы и расскажет о городе</span></div>'
                )
                return
            ...
            for msg in state["council_chat"]:
                ...
                ui.html(
                    f'<div style="margin:8px 0;padding:12px 16px;background:{bg};border:1px solid {border};border-radius:8px;"><div style="font-family:JetBrains Mono;font-size:0.55rem;color:rgba(140,150,180,0.5);margin-bottom:6px;">{speaker or role} · {time_}</div><div style="font-family:JetBrains Mono;font-size:0.75rem;color:{color};line-height:1.55;white-space:pre-wrap;">{escaped}</div></div>'
                )
        ui.run_javascript(
            'const el=document.querySelector(".council-chat-scroll");if(el) el.scrollTop=el.scrollHeight;'
        )
            ...
            with ui.element('div').classes('cab-right'):
                with ui.element('div').classes('cab-panel-title').style(
                    ...
                    with ui.element('div').style(
                        ...
                        ui.button('Совет', on_click=lambda: set_center_view('council')).props('flat dense').style(
                            'font-family:JetBrains Mono; font-size:0.55rem;'
                            'letter-spacing:0.06em; color:rgba(180,190,220,0.6);'
                            'padding:2px 8px; border-radius:4px;'
                            'background:rgba(99,130,255,0.08);'
