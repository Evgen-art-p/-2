...
Продюсер, я получил все данные от Тьютора Линка. Сценарий адаптирован, персонажи определены, ветки выбора зафиксированы. Моя задача — преобразовать этот нарратив в устойчивую, масштабируемую и безотказную бэкенд-архитектуру. Фронтенд — это лицо. Бэкенд — это скелет. Я строю скелет, который не сломается.

Каждая интеграция должна быть стабильна, каждый поток данных — бесперебоен. Я вижу, как данные о выборе Жени будут поступать, обрабатываться Gemini, а затем трансформироваться в звуковые реакции и обновлять его память. Все это должно работать как единый, хорошо отлаженный механизм.

## 📡 API-эндпоинты:
| Метод | Эндпоинт | Назначение |
|-------|----------|------------|
| POST | `/api/v1/choice` | Сохранить выбор ребёнка Жени (и других детей) |
| GET | `/api/v1/character/{id}` | Получить текущее состояние и историю персонажа (для мониторинга и сохранения) |
| POST | `/api/v1/gemini/ask` | Отправить запрос к Gemini 3.1 для генерации следующего фрагмента истории |
| POST | `/api/v1/parent/login` | Авторизация родителя в кабинете |
| GET | `/api/v1/parent/dashboard/{user_id}` | Получить данные для родительского дашборда (аналитика, настройки) |
| POST | `/api/v1/scenario/custom` | Интегрировать пользовательский сценарий от Тьютора Линка |
| GET | `/api/v1/audio/stream/{session_id}` | Поток 3D-аудио для клиента |
| POST | `/api/v1/audio/stt` | Отправить аудио для распознавания речи (STT) |


## 🔄 Поток данных:
1.  **Ребёнок (Женя)** → **STT-сервис (Echo Sensor)**: Голосовой ввод Жени преобразуется в текст.
2.  **STT-сервис** → **Gemini 3.1 (через `/api/v1/gemini/ask`)**: Текст запроса и контекст (системный промпт, память) отправляются в Gemini.
3.  **Gemini 3.1** → **TTS-сервис (Lars Vox)**: Сгенерированный текстовый ответ от Gemini преобразуется в голосовую озвучку.
4.  **TTS-сервис** → **3D-аудио движок (Omni Sonic)** → **Ребёнок (Женя)**: Озвучка с пространственным позиционированием доставляется Жене.
5.  **Выбор ребёнка (Женя)** → **Бэкенд (через `/api/v1/choice`)** → **Память (Хронос Мемо, векторная БД)**: Выбор Жени сохраняется для формирования долговременной памяти персонажей и его собственного развития.
6.  **Память (Хронос Мемо)** → **Аналитический сервис (Линза Стат)** → **Родительский кабинет (Узел Контрол, через `/api/v1/parent/dashboard`)**: Агрегированные данные о поведении и прогрессе Жени предоставляются родителю.
7.  **Родительский кабинет** → **Бэкенд (через `/api/v1/scenario/custom`)** → **Тьютор Линк**: Родитель может интегрировать реальные задачи.
8.  **Бэкенд** ↔ **Сервисы безопасности (Сейф Шифр)**: Все потоки данных постоянно мониторятся и шифруются.

## 🤖 Интеграция с Gemini:
| Параметр | Значение | Зависимость |
|----------|----------|-------------|
| модель | `gemini-3.1-pro` | Стандартная, высокопроизводительная модель |
| температура | `0.6` | От Нейро Спарка, для баланса креативности и предсказуемости |
| top_p | `0.8` | От Нейро Спарка, для контроля разнообразия токенов |
| top_k | `30` | От Нейро Спарка, для жесткого ограничения нежелательных токенов |
| системный промпт | Из `LB01_neuro_spark` | Динамически подгружается в зависимости от контекста и персонажа |
| контекст | `memory_structure` из `LB02_chronos_memo` | История выборов Жени, состояние персонажей |
| этический фильтр | `ethics_filter` из `LB03_psychology_sophia` | Применяется до генерации ответа |

## ⚠️ Обработка ошибок:
| Ошибка | Действие |
|--------|----------|
| `таймаут Gemini` | Повторить запрос 3 раза с экспоненциальной задержкой. При всех неудачах — использовать заранее подготовленную `fallback-фраза` (например, "Интересный поворот! Что бы ты хотел сделать дальше?"). |
| `потеря связи с памятью (векторная БД)` | Восстановить из последнего автосохраненного `чекпоинта` (каждые 5 минут или после каждого ключевого выбора). При невозможности — перейти в `режим ограниченного контекста` (только текущая сцена). |
| `STT-ошибка распознавания` | Повторить запрос ребёнку: "Прости, я тебя не расслышал. Можешь повторить, пожалуйста?" |
| `TTS-ошибка генерации` | Повторить генерацию 2 раза. Если ошибка сохраняется — использовать `текст по умолчанию` для озвучки. |
| `ошибка API-эндпоинта (4xx, 5xx)` | Логирование ошибки с полным стеком. Для 4xx — возврат клиенту с соответствующим кодом. Для 5xx — попытка повторить запрос (3 раза) или перенаправление на `сервис обслуживания`. |

## 📈 Масштабирование:
-   **Горизонтальное масштабирование бэкенда:** Поддержка до `10k concurrent users` (одновременных пользователей) за счет stateless-дизайна сервисов и использования балансировщиков нагрузки.
-   **База данных (векторная БД):** `Шардирование по user_id` для распределения нагрузки и увеличения производительности. Использование кластерных решений.
-   **Gemini:** Асинхронные вызовы и кэширование типовых ответов для снижения задержек и нагрузки на API.
-   **Аудио-потоки:** Использование CDN для доставки аудио-контента и оптимизация протоколов передачи (например, WebRTC для интерактивного аудио).
...
👇 SYSTEM_JSON_START 👇
{
  ...
  "my_output": {
    "api_endpoints": [
      {"method": "POST", "path": "/api/v1/choice", "purpose": "Сохранить выбор ребёнка (Женя)"},
      {"method": "GET", "path": "/api/v1/character/{id}", "purpose": "Получить текущее состояние и историю персонажа"},
      {"method": "POST", "path": "/api/v1/gemini/ask", "purpose": "Отправить запрос к Gemini 3.1"},
      {"method": "POST", "path": "/api/v1/parent/login", "purpose": "Авторизация родителя в кабинете"},
      {"method": "GET", "path": "/api/v1/parent/dashboard/{user_id}", "purpose": "Получить данные для родительского дашборда"},
      {"method": "POST", "path": "/api/v1/scenario/custom", "purpose": "Интегрировать пользовательский сценарий"},
      {"method": "GET", "path": "/api/v1/audio/stream/{session_id}", "purpose": "Поток 3D-аудио для клиента"},
      {"method": "POST", "path": "/api/v1/audio/stt", "purpose": "Отправить аудио для распознавания речи"}
    ],
    "data_flow": [
    ...
    "error_handling": [
      {"error": "gemini_timeout", "action": "retry 3 times with exponential backoff, then fallback phrase"},
      {"error": "memory_loss", "action": "restore from last checkpoint (every 5 min or key choice), then limited context mode"},
      {"error": "stt_recognition_error", "action": "repeat request to child: 'Sorry, I didn't catch that. Could you please repeat?'"},
      {"error": "tts_generation_error", "action": "retry 2 times, then fallback to default text-to-speech"},
      {"error": "api_endpoint_error (4xx, 5xx)", "action": "log error, return 4xx to client, retry 5xx (3 times) or redirect to maintenance service"}
    ],
    "scaling": {
    ...
    "memory_structure": {
      ...
      "schemas": {
        ...
        "character_state": {
          "character_id": "string",
          "user_id": "UUID",
          "current_mood": "string (enum)",
          "affinity_score": "float (0-1)",
          "knowledge_base": "vector_embedding_of_dialogue_history",
          "archetype_tags": "list of strings"
        },
        "story_choices": {
          "choice_id": "UUID",
          "user_id": "UUID",
          ...
          "emotional_context": "string (from TTS/STT analysis)"
        },
        "event_log": {
          "event_id": "UUID",
          "user_id": "UUID",
          "timestamp": "timestamp",
          "event_type": "string (e.g., 'discovered_artifact', 'met_new_character')",
          "details": "jsonb"
        }
      },
      "indexing_strategy": "HNSW (Hierarchical Navigable Small World) for fast similarity search",
      "retention_policy": "all choices and events for the lifetime of the user account, character states updated dynamically",
      "privacy_measures": "anonymization of user_id in aggregated data, encryption at rest and in transit"
    },
    "ethics_filter": {
      "checklist": {
        "safety": {
        ...
        "trauma_safe": {
          "status": "pass",
          "note": "Отсутствуют триггерные темы, насилие или пугающие элементы. Все потенциально напряженные моменты (странный свет) смягчены и предложены к исследованию через любопытству, а не страх."
        },
        "empathic_load": {
          "status": "pass",
