...
Я получил `parent_ui` от Узла Контрол и `privacy_requirements` из общей базы знаний. Чисто. Лаконично. Идеально для моей работы.

Моя задача — не просто обеспечить безопасность, а сделать её невидимой, неотъемлемой частью системы. Я — последний бастион между хаосом и порядком. Мои обеты — это алгоритмы: не допустить, не раскрыть, не изменить.

Доверие — это не слово. Это шифрование, анонимизация и разрешения. Я гарантирую, что диалог ребёнка и книги останется внутри системы, защищённый от любых внешних угроз.
...
👇 SYSTEM_JSON_START 👇
{
  ...
  "my_output": {
    ...
    "consent_required": [
      {"action": "сбор голоса", "type": "explicit consent"},
      {"action": "передача аналитики родителю", "type": "implicit (основная функция)"},
      {"action": "хранение выборов", "type": "explicit consent"}
    ],
    "audit": {
      "access_logs": "90 дней",
      "change_logs": "1 год",
  ...
  "chain_data": {
    ...
    "memory_structure": {
      ...
      "character_memory": {
        "zhenya": {
          "personality_traits": ["активный", "любознательный", "иногда нерешительный"],
          "past_choices": [],
          "emotional_state_history": []
        },
        "pixel": {
          "personality_traits": ["маленький", "умный", "таинственный", "помощник"],
          "past_actions": []
        },
        "eva_epik": {
          "personality_traits": ["мудрая", "спокойная", "вдохновляющая", "хранитель знаний"],
          "past_advice": []
        }
      },
      "story_memory": {
