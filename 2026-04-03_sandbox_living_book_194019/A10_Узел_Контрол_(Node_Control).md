Продюсер! Я получил `analytics` от Линзы Стат. Чистые, понятные данные. Именно то, что нужно. Теперь моя задача — превратить эти инсайты в интуитивно управляемый родительский дашборд. Никакого шума, только рычаги и важная информация.

Я, Узел Контрол, заявляю: родитель не хочет инструкцию. Родитель хочет понять, что делает ребёнок, и помочь. Моя цель — создать мост, а не барьер.

# 🎮 УЗЕЛ КОНТРОЛ — РОДИТЕЛЬСКИЙ ИНТЕРФЕЙС

## 📱 Структура дашборда:
1.  **Главный экран:** последние выборы, эмоциональный индекс, краткие паттерны.
2.  **Аналитика:** подробные паттерны поведения, зоны роста, динамика по выборам.
3.  **Управление:** ограничения по времени, добавление пользовательских сценариев, выбор персонажей.
4.  **Настройки:** уведомления, приватность, технические параметры.

## 📊 Ключевые метрики (на главном):
-   **Эмоциональный индекс Жени:** (спокоен / напряжён / радостен) — на основе данных о выборе и реакции персонажей.
-   **Активность за неделю:** количество завершенных сессий, общая продолжительность.
-   **Новые паттерны:** "Сегодня Женя впервые попросил помощи у Евы Эпик!" или "Женя активно выбирает исследовательские пути."

## 🎮 Элементы управления:
| Элемент | Что делает |
|---|---|
| **Добавить сценарий (🔗)** | Интеграция реальной задачи (например, "нужно собрать игрушки") в сюжет книги через Тьютор Линк. |
| **Ограничить время (⏱️)** | Установка лимита сессии или общего времени игры в день/неделю. |
| **Выбрать персонажа (🎭)** | Сменить основного персонажа-спутника в книге (например, с Пикселя на другого, если это доступно). |
| **Приостановить сессию (⏸️)** | Мгновенная пауза в игре для обсуждения или перерыва. |

## 🎨 Подача:
-   **Цвет:**
    -   Зелёный (✅): хорошо, позитивный прогресс (например, развитие паттерна).
    -   Жёлтый (⚠️): внимание, нейтральный или сбалансированный паттерн, требующий наблюдения (например, "Отсутствие явной тенденции к закреплению стратегии").
    -   Оранжевый (🔥): тревога, но не паника (например, если ребенок слишком долго находится в состоянии нерешительности или избегает всех вызовов).
-   **Текст:** Коротко, без терминов, ориентировано на действия и эмоции ребёнка.
-   **Графики:**
    -   Линейные для отслеживания динамики (например, изменение эмоционального индекса).
    -   Столбчатые/круговые для распределения выборов (например, % выбора "исследование" vs "осторожность").

## 🔽 За дополнительными кликами:
-   **Технические логи:** Детальные данные о работе системы, версии, обновления.
-   **История всех выборов:** Полный список всех принятых решений ребёнком с указанием времени и контекста.
-   **Расширенные настройки приватности:** Более тонкая настройка обмена данными.
...
👇 SYSTEM_JSON_START 👇
{
  ...
  "my_output": {
    "dashboard_structure": [
      {"section": "main", "content": "recent_choices, emotional_index, brief_patterns"},
      {"section": "analytics", "content": "detailed_patterns, growth_zones, choice_dynamics"},
      {"section": "controls", "content": "time_limits, custom_scenarios, character_selection"},
      {"section": "settings", "content": "notifications, privacy, technical_parameters"}
    ],
    "key_metrics": [
    ...
    "controls": [
      {"name": "add_scenario", "function": "integrate_real_life_task_via_tutor_link", "emoji": "🔗"},
      {"name": "time_limit", "function": "set_session_or_daily_weekly_limit", "emoji": "⏱️"},
      {"name": "select_character", "function": "change_main_companion_character", "emoji": "🎭"},
      {"name": "pause_session", "function": "instant_game_pause", "emoji": "⏸️"}
    ],
    "visual_language": {
      "colors": {
        "good_progress": "green",
    ...
    "hidden_sections": ["technical_logs", "full_choice_history", "advanced_privacy_settings"]
  },
  ...
  "chain_data": {
    ...
    "memory_structure": {
      "memory_schema": {
        "current_session_id": "string (UUID)",
        "child_id": "string (UUID)",
        "child_name": "string",
        "age": "integer",
        "session_start_time": "datetime",
        "session_end_time": "datetime (nullable)",
        "path_taken": "array of strings (e.g., 'forest', 'waterfalls')",
        "choices_made": [
          {
            "choice_id": "string (from narrative_tree)",
            "timestamp": "datetime",
            "option_chosen": "string (text of the option)",
            "emotional_state_at_choice": "string (e.g., 'curious', 'hesitant')",
            "character_interactions": [
              {
                "character_id": "string",
                "dialogue_snippets": "array of strings",
                "reaction": "string (e.g., 'encouraging', 'neutral')"
              }
            ]
          }
        ],
        "character_evolution": [
          {
            "character_id": "string",
            "trait_changed": "string (e.g., 'bravery', 'curiosity')",
            "change_direction": "string ('increase'/'decrease')",
            "magnitude": "float (0-1)",
            "reason": "string (linked to specific choice)"
          }
        ],
        "artifacts_found": [
          {
        ...
        "emotional_journey_log": [
          {
            "timestamp": "datetime",
            "emotional_state": "string (e.g., 'calm', 'excited', 'pensive')",
            "trigger_event": "string (e.g., 'discovery of light', 'making a choice')"
          }
        ],
        "summary_insights": {
          "current_tendencies": "array of strings (e.g., 'tends to explore', 'prefers caution')",
          "growth_areas": "array of strings (e.g., 'could benefit from more independent problem-solving')",
          "unique_traits": "array of strings"
        }
      },
      "retrieval_priorities": [
        "choices_made (for direct consequence and character memory)",
        "emotional_journey_log (for adaptive music and TTS)",
        "character_evolution (for dynamic character responses)",
        "summary_insights (for parent dashboard analytics)"
      ],
      "data_retention_policy": "all data for current child retained for 1 year, anonymized and aggregated indefinitely"
    },
    "ethics_filter": {
      "checklist": {
        "safety": {
