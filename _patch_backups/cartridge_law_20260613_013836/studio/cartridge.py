...
class CartridgeManifest:
    ...
    # Hard stop — резиденты на checkpoint
    # {"after_agent": "A04", "residents": ["victor"]}
...
class PipelineCallbacks:
    ...
    async def on_victor_ready(self, slot_id: str, critique: dict):
        """Виктор завершил критику — активировать пузырёк в UI."""
        pass
...
class CartridgeRunner:
    ...
    async def run(
        ...
        # При CONTINUE (from_worker) стартуют только оставшиеся агенты.
        _work_agents = all_agents[start_index:]
        self._log_work_status("start", _work_agents)
        ...
        while i < len(all_agents):
            ...
            try:
                ...
                if (
                    ...
                    # ПАТЧ city_red №3: пауза Виктора закрывает смену —
                    # CONTINUE откроет новый work_start для оставшихся
                    self._log_work_status("end", _work_agents, status="PAUSED")
                ...
                if worker_id in self.manifest.checkpoint_after:
                    ...
                    if not should_continue:
                        ...
                        # ПАТЧ city_red №3: checkpoint-пауза закрывает смену
                        self._log_work_status("end", _work_agents, status="PAUSED")
                        return
        ...
        # Пайплайн завершён
        # ── ЧЕСТНЫЙ РАБОЧИЙ СТАТУС: смена закончилась · ПАТЧ city_red №3 ──
        # Покрывает и нормальный финал, и break (stop_after / action=stop).
        self._log_work_status("end", _work_agents, status="DONE")
        ...
        # update_slot_field уже вызывается в hooks.py финализатором (A12) для цехов с hooks.
        # Для цехов без hooks.py (turbo, legacy) — вызываем здесь.
        if not self._hooks:
            try:
    ...
    async def run_turbo(self):
        ...
        self._log_work_status("start", list(turbo_agents))
            ...
            if parallel_group and worker_id == parallel_group[0]:
                ...
                for j, wid in enumerate(parallel_group):
                    ...
                    if isinstance(results[j], Exception):
                        ...
                        h_text, previous_output, ghost_ids = await process_agent_result(
        ...
        # ПАТЧ city_red №3: турбо-смена закончилась
        self._log_work_status("end", list(turbo_agents), status="DONE")
        await self.callbacks.on_pipeline_done(self.slot_id, self.state.get("results", {}))
    ...
    def _log_work_status(self, event: str, agents: list, status: str = ""):
        """
        ПАТЧ city_red №3 · Честный рабочий статус на уровне шасси.

        Один писатель для ВСЕХ картриджей: work_start при старте смены,
        work_end на ЛЮБОМ выходе (DONE / PAUSED / stop / break).
        Закрывает дыру: BIBLE-флоу и хард-стопы оставляли work_start
        незакрытым на 8 часов — walk_rest заморожен, на карте все «в Студии».
        Дубли из hooks.py цехов безвредны: тот же slot_id/project_id,
        is_agent_working дедуплицирует по нему.
        """
        try:
            from studio.city_pulse import log_work_start, log_work_end
            pid = self.state.get("project_id", "")
            for aid in agents:
                if event == "start":
                    log_work_start(agent=aid, dept=self.manifest.id,
                                   slot_id=self.slot_id, project_id=pid)
                else:
                    log_work_end(agent=aid, dept=self.manifest.id,
                                 slot_id=self.slot_id, project_id=pid,
                                 status=status or "DONE")
            _tag = "🏭" if event == "start" else "🏁"
            print(f"[CARTRIDGE] {_tag} work_{event} → "
                  f"{len(agents)} агентов {self.manifest.id}"
                  + (f" · {status}" if status else ""))
        except Exception as _we:
            print(f"[CARTRIDGE] ⚠ work_{event}: {_we}")
